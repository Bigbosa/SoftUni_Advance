...
for i in range(lines):
    ...
    for i in range(range_2[0], range_2[1] + 1):
        set_b.add(i)
    sets_interaction = set_a.intersection(set_b)
    if len(sets_interaction) > max_intersection:
        max_intersection = len(sets_interaction)
        combined_intersection = sets_interaction
    set_a.clear()
    set_b.clear()
